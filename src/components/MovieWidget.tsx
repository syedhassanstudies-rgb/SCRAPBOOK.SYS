...
export function MovieWidget({ id, userId, title, rating, year, rotation = -2, color = 'secondary', posterUrl: initialPosterUrl, bgColor, fontFamily, borderStyle, theme = 'retro' }: MovieWidgetProps) {
  ...
  return (
    <motion.article
      ...
      style={{ backgroundColor: theme === 'y2k' ? undefined : (bgColor || '#fdfbf9') }}
    >
      {theme === 'retro' && <Tape color={color as any} rotation={-15} className="-top-3 -left-2 w-16 h-5 opacity-90" />}
      ...
      <div className="flex items-start gap-4 mb-4">
        <div className={`w-24 h-36 ${theme === 'y2k' ? 'bg-fuchsia-500/10 border-fuchsia-300 rounded-lg shadow-inner' : `bg-paper-outline/10 border ${borderColor}`} flex-shrink-0 relative overflow-hidden shadow-sm ${theme === 'retro' ? 'rotate-2' : ''} group-hover:rotate-0 transition-transform duration-500`}>
           {theme === 'retro' && <Tape color="primary" rotation={0} className="-top-2 left-1/2 -translate-x-1/2 w-8 h-3 opacity-50" />}
           {loading ? (
        ...
        <div className="flex flex-col flex-grow">
          <div className={`flex items-center justify-between mb-2 border-b-2 ${theme === 'y2k' ? 'border-pink-300 border-dashed text-fuchsia-600' : borderColor} pb-1`}>
             <h3 className={`${theme === 'retro' ? 'font-serif italic tracking-tight leading-tight' : ''} text-xl`}>{title}</h3>
          </div>
          ...
          <div className="flex justify-between items-end mt-auto font-mono">
             ...
             <div className={`${theme === 'minimal' ? 'bg-gray-100 text-gray-900 rounded-md border border-gray-200' : theme === 'y2k' ? 'bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white rounded-full shadow-[0_0_10px_rgba(255,105,180,0.5)]' : 'bg-paper-ink text-white'} px-2 py-1 ${theme === 'retro' ? '-rotate-2' : ''}`}>
      ...
      {theme === 'retro' && <div className={`absolute top-0 right-4 bottom-0 w-px border-l-2 border-dashed ${borderColor}`} />}
    </motion.article>
  );
}
